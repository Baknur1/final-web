...
class ItemRepository {
    async findAll() { return await Item.find().populate('user_id warehouse_id'); }
    async findById(id) { return await Item.findById(id).populate('user_id warehouse_id'); }
    async find(query) { return await Item.find(query).populate('user_id warehouse_id'); }
    async save(data) {
        if (data._id || data.id) {
            const id = data._id || data.id;
